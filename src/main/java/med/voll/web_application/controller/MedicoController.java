...
public class MedicoController {
    ...
    @GetMapping
    public String carregarPaginaListagem(@PageableDefault Pageable paginacao, Model model, @AuthenticationPrincipal Usuario userLogado) {
        if (userLogado.getPerfil() == Perfil.MEDICO) {
            return PAGINA_ERRO;
        }
    ...
    public String carregarPaginaCadastro(Long id, Model model, @AuthenticationPrincipal Usuario userLogado) {
        if (userLogado.getPerfil() != Perfil.ATENDENTE) {
            return PAGINA_ERRO;
        }
    ...
    @PostMapping
    public String cadastrar(@Valid @ModelAttribute("dados") DadosCadastroMedico dados, BindingResult result, Model model, @AuthenticationPrincipal Usuario userLogado) {
        if (userLogado.getPerfil() != Perfil.ATENDENTE) {
            return PAGINA_ERRO;
        }
    ...
    @DeleteMapping
    public String excluir(Long id, @AuthenticationPrincipal Usuario userLogado) {
        if (userLogado.getPerfil() != Perfil.ATENDENTE) {
            return PAGINA_ERRO;
        }
