    ...
    public SecurityFilterChain filtrosSeguranca(HttpSecurity http, OncePerRequestFilter filtroAlteracaoSenha) throws Exception {
        return http
                .authorizeHttpRequests(req -> {
                        req.requestMatchers("/css/**", "/js/**", "/assets/**", "/", "/index", "/home", "/esqueci-senha", "/recuperar-conta").permitAll();
...
//                        req.requestMatchers(HttpMethod.GET, "/consultas/**").hasAnyRole("MEDICO", "ATENDENTE", "PACIENTE");
//                        req.requestMatchers(HttpMethod.DELETE, "/consultas/**").hasAnyRole("MEDICO", "ATENDENTE", "PACIENTE");
